...
def find_chrome():
    configured = (
        os.environ.get("CLOAKBROWSER_EXECUTABLE_PATH")
        or os.environ.get("XAI_ENROLLER_BROWSER_EXECUTABLE")
        or ""
    ).strip().strip('"')
    if configured:
        ...
        if os.path.isfile(configured):
            return configured
        raise RuntimeError(f"Configured CloakBrowser executable not found: {configured}")

    roots = [os.path.expanduser("~/.cloakbrowser")]
    local_app_data = os.environ.get("LOCALAPPDATA")
    if local_app_data:
        roots.append(os.path.join(local_app_data, "cloakbrowser"))

    paths = []
    for root in roots:
        paths.extend(glob.glob(os.path.join(root, "chromium-*", "chrome")))
        paths.extend(glob.glob(os.path.join(root, "chromium-*", "chrome.exe")))
        paths.extend(
            glob.glob(
                os.path.join(
                    root,
        ...
        paths.extend(glob.glob(os.path.join(root, "**", "chrome.exe"), recursive=True))

    paths = [path for path in paths if os.path.isfile(path)]
    if not paths:
        raise RuntimeError(
            "CloakBrowser not found. Run setup.ps1 or set "
...
def reap_orphan_register_browsers(*, exclude_pids: set[int] | None = None) -> int:
    ...
    """
    if os.name == "nt":
        # browser.close() handles the Windows process tree. Avoid a broad taskkill
        # scan because Windows does not expose Linux-style /proc ownership data.
        return 0
...
def get_system_resources(max_mem_arg=None):
    ...
    if os.name == "nt":
        try:
            ...
            total = int(status.total_physical // (1024 * 1024))
            available = int(status.available_physical // (1024 * 1024))
        except Exception:
...
def create_email():
    ...
    password = rand_str()
    # 优先 TempMail.lol(mail.tm 的 /token POST 在本机 fake-IP 隧道下 SSL 握手僵死、
    # req.post 的 timeout 不覆盖 _create 阶段 → 卡住永不 raise → try/except fallback
    # 够不着「卡死」状态 → 第一 maker 把整条 P 焊死。故 lol 先跑,mail.tm 后置兜底)。
    makers = [_lol_create] + [(lambda b=b: _mailtm_create(b, password)) for b in TEMPMAIL_BASES]
    for make in makers:
        try:
            handle, email = make()
...
def _persist_registration(email, password, sso, session_cookies):
    if session_cookies:
        ...
        _append_registration_line(
            _registration_output_path("auth-sessions.jsonl"),
            document + "\n",
            mode=0o600,
            durable=True,
        )
    _append_registration_line(
        _registration_output_path("accounts.txt"),
        f"{email}:{password}:{sso}\n",
    )
    _append_registration_line(_registration_output_path("grok.txt"), sso + "\n")
